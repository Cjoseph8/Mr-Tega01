...
const {token} = req.params
 ...
 // Direct the user to their Mail
 if(user.isVerified === true){
    return res.status(400).json({message:'user already verified, please check your email'})
 }
...
//         const findUser = await schoolModel.findOne({$or:[{name:emailOrUserName},{email:emailOrUserName}]})
//         if(!findUser){
//             return res.status(404).json({message:'user with this email does not exist'})
//         }
//         const matchedPassword = await bcrypt.compare(password, findUser.password)
